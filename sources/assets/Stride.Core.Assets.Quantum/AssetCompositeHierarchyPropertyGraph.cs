    ...
    where TAssetPartDesign : class, IAssetPartDesign<TAssetPart>
{
    ...
    /// </summary>
    private readonly Dictionary<AssetCompositeHierarchyPropertyGraph<TAssetPartDesign, TAssetPart>, HashSet<Guid>> basePartAssets = [];
    /// <summary>
    ...
    private readonly Dictionary<Tuple<Guid, Guid>, TAssetPartDesign> baseInstanceMapping = [];
    ...
    /// </summary>
    private readonly HashSet<Tuple<Guid, Guid>> deletedPartsInstanceMapping = [];
    /// <summary>
    ...
    private readonly Dictionary<Guid, Guid> instancesCommonAncestors = [];
    /// <summary>
    /// A hashset of nodes representing the collections of children from a parent part.
    /// </summary>
    private readonly HashSet<IGraphNode> registeredChildParts = [];
    ...
    public IReadOnlyCollection<Guid> GetInstanceIds(AssetCompositeHierarchyPropertyGraph<TAssetPartDesign, TAssetPart> baseAssetPropertyGraph)
    {
        ArgumentNullException.ThrowIfNull(baseAssetPropertyGraph);
        basePartAssets.TryGetValue(baseAssetPropertyGraph, out var instanceIds);
        return (IReadOnlyCollection<Guid>?)instanceIds ?? [];
    }
    ...
    public void BreakBasePartLinks(IEnumerable<TAssetPartDesign> assetPartDesigns)
    {
        foreach (var part in assetPartDesigns.Where(x => x.Base is not null))
        {
    ...
    public void AddPartToAsset(AssetPartCollection<TAssetPartDesign, TAssetPart> newPartCollection, TAssetPartDesign child, TAssetPart? parent, int index)
    {
    ...
    public void RemovePartFromAsset(TAssetPartDesign partDesign)
    {
        if (!Asset.Hierarchy.RootParts.Contains(partDesign.Part))
        {
            var parent = Asset.GetParent(partDesign.Part)
                ?? throw new InvalidOperationException("The part has no parent but is not in the RootParts collection.");
    ...
    public void DeleteParts(IEnumerable<TAssetPartDesign> partDesigns, out HashSet<Tuple<Guid, Guid>> deletedPartsMapping)
    {
        ArgumentNullException.ThrowIfNull(partDesigns);
        ...
        while (partsToDelete.Count > 0)
        {
            ...
            if (partToDelete.Base is not null)
            {
    ...
    public override IGraphNode FindTarget(IGraphNode sourceNode, IGraphNode target)
    {
        ...
        if (sourceNode.Retrieve() is TAssetPart part && sourceNode is IObjectNode)
        {
            ...
            if (Asset.Hierarchy.Parts.TryGetValue(part.Id, out var partDesign) && partDesign.Base is not null)
            {
                var baseAssetGraph = Container.TryGetGraph(partDesign.Base.BasePartAsset.Id);
                // Base asset might have been deleted
                if (baseAssetGraph is null)
                ...
                return basePart is not null ? Container.NodeContainer.GetOrCreateNode(basePart.Part) : base.FindTarget(sourceNode, target);
            }
        }
    ...
    public static AssetCompositeHierarchyData<TAssetPartDesign, TAssetPart> CloneSubHierarchies(AssetNodeContainer nodeContainer, AssetCompositeHierarchy<TAssetPartDesign, TAssetPart> asset,
        IEnumerable<Guid> sourceRootIds, SubHierarchyCloneFlags flags, out Dictionary<Guid, Guid> idRemapping)
    {
    ...
    /// <returns>A <see cref="AssetCompositeHierarchyData{TAssetPartDesign, TAssetPart}"/> corresponding to the cloned parts.</returns>
    /// <remarks>The parts passed to this methods must be independent in the hierarchy.</remarks>
    public static AssetCompositeHierarchyData<TAssetPartDesign, TAssetPart> CloneSubHierarchies(AssetNodeContainer sourceNodeContainer, AssetNodeContainer targetNodeContainer,
        AssetCompositeHierarchy<TAssetPartDesign, TAssetPart> asset, IEnumerable<Guid> sourceRootIds, SubHierarchyCloneFlags flags, out Dictionary<Guid, Guid> idRemapping)
    {
        ArgumentNullException.ThrowIfNull(sourceNodeContainer);
        ArgumentNullException.ThrowIfNull(targetNodeContainer);
        ArgumentNullException.ThrowIfNull(asset);
        ArgumentNullException.ThrowIfNull(sourceRootIds);
        ...
        {
            if (!asset.Hierarchy.Parts.TryGetValue(rootId, out var item))
                throw new ArgumentException("The source root parts must be parts of this asset.", nameof(sourceRootIds));

            subTreeHierarchy.RootParts.Add(item.Part);

            subTreeHierarchy.Parts.Add(item);
            foreach (var subTreePart in asset.EnumerateChildParts(item.Part, true))
    ...
    public void TrackDeletedInstanceParts(IEnumerable<Tuple<Guid, Guid>> deletedPartsMapping)
    {
        ArgumentNullException.ThrowIfNull(deletedPartsMapping);
    ...
    /// </summary>
    /// <param name="deletedPartsMapping">A mapping of deleted parts (base part id, instance id).</param>
    public void UntrackDeletedInstanceParts(IEnumerable<Tuple<Guid, Guid>> deletedPartsMapping)
    {
        ArgumentNullException.ThrowIfNull(deletedPartsMapping);
    ...
    protected override void FinalizeInitialization()
    {
        ...
        foreach (var kv in basePartAssets)
        {
            ...
            var baseParts = baseAsset.Hierarchy.Parts.Keys.SelectMany(_ => instanceIds, Tuple.Create);
    ...
    protected abstract void AddChildPartToParentPart(TAssetPart parentPart, TAssetPart childPart, int index);
    ...
    /// <param name="childPart">The child part.</param>
    /// <remarks>This method does not modify the <see cref="AssetCompositeHierarchyData{TAssetPartDesign, TAssetPart}"/> contained in this asset.</remarks>
    protected abstract void RemoveChildPartFromParentPart(TAssetPart parentPart, TAssetPart childPart);
    ...
    /// </remarks>
    protected virtual void ReuseExistingPart(AssetCompositeHierarchyData<TAssetPartDesign, TAssetPart> baseHierarchy, TAssetPartDesign clonedPart, TAssetPartDesign existingPart)
    {
    ...
    protected virtual int FindBestInsertIndex(AssetCompositeHierarchy<TAssetPartDesign, TAssetPart> baseAsset, TAssetPartDesign newBasePart, TAssetPart? newBasePartParent, Guid instanceId, out TAssetPartDesign? instanceParent)
    {
        ...
        if (newBasePartParent is null)
        {
            ...
            for (var i = partIndex - 1; i >= 0 && insertIndex < 0; --i)
            {
                ...
                if (instanceSibling is not null)
                {
                    ...
                    instanceParent = parent is not null ? Asset.Hierarchy.Parts[parent.Id] : null;
        ...
        if (insertIndex < 0)
        {
            ...
            if (isAlone)
            {
                if (instancesCommonAncestors.TryGetValue(instanceId, out var parentId) && parentId != Guid.Empty)
                {
    ...
    private void RelinkToOwnerPart(IAssetNode node, object? newValue)
    {
        ...
        if (partDesign is not null)
        {
            // A property of a part has changed
            LinkToOwnerPart(node, partDesign);
        }
        else if (node.Type == typeof(AssetPartCollection<TAssetPartDesign, TAssetPart>) && newValue is TAssetPartDesign design)
        {
        ...
        foreach (var part in currentParts)
        {
            ...
            else
            {
                ...
                while (parent != Guid.Empty)
                {
                    ...
                    parent = Asset.Hierarchy.Parts.TryGetValue(parent, out var assetPartDesign) ? Asset.GetParent(assetPartDesign.Part)?.Id ?? Guid.Empty : Guid.Empty;
                }
                ...
                while (ancestorId != Guid.Empty && !parents.Contains(ancestorId))
                {
                    ...
                    ancestorId = Asset.Hierarchy.Parts.TryGetValue(ancestorId, out var assetPartDesign) ? (Asset.GetParent(assetPartDesign.Part)?.Id ?? Guid.Empty) : Guid.Empty;
                }
    ...
    private void PartAddedInBaseAsset(object? sender, AssetPartChangeEventArgs e)
    {
        ...
        foreach (var instanceId in basePartAssets[baseAssetGraph])
        {
            ...
            foreach (var ids in mapping)
            {
                ...
                if (!deletedPartsInstanceMapping.Contains(mappingKey) && baseInstanceMapping.TryGetValue(mappingKey, out var existingPart))
                {
    ...
    private void RootPartsChanged(object? sender, INodeChangeEventArgs e)
    {
        switch (e.ChangeType)
        {
            case ContentChangeType.CollectionAdd:
                {
                    if (e.NewValue is TAssetPart part)
                        NotifyPartAdded(part.Id);
                }
                break;
            case ContentChangeType.CollectionRemove:
                {
                    if (e.OldValue is TAssetPart part)
                        NotifyPartRemoved(part.Id);
                }
                break;
        }
    }
