...
public class AssetPropertyGraph : IDisposable
{
    ...
    public readonly struct NodeOverride
    {
    ...
    private readonly struct NodeChangeHandlers
    {
    ...
    private readonly Dictionary<IGraphNode, OverrideType> previousOverrides = [];
    private readonly Dictionary<IGraphNode, ItemId> removedItemIds = [];
    ...
    private readonly Dictionary<IAssetNode, NodeChangeHandlers> baseLinkedNodes = [];
    ...
    private IBaseToDerivedRegistry BaseToDerivedRegistry => baseToDerivedRegistry ??= CreateBaseToDerivedRegistry();
    ...
    private void ReconcileWithBase(IAssetNode rootNode, Dictionary<IGraphNode, NodeIndex>? nodesToReset = null)
    {
    ...
    internal void ResetAllOverridesRecursively(IAssetNode rootNode, NodeIndex indexToReset)
    {
        ...
        if (visitRoot is not null)
        {
            ...
            visitor.Visiting += (node, _) => nodesToReset.TryAdd(node, NodeIndex.Empty);
            visitor.Visit(rootNode);
        }
    ...
    public virtual void ClearReferencesToObjects(IEnumerable<Guid> objectIds)
    {
    ...
    private static IAssetNode? ResolveObjectPath(IAssetNode rootNode, YamlAssetPath path, out NodeIndex index, out bool resolveOnIndex)
    {
    ...
    private static object? CloneFromBase(object value)
    {
        ...
        return AssetCloner.Clone<object>(value, AssetClonerFlags.GenerateNewIdsForIdentifiableObjects, out _);
    }
    ...
    private void AssetItemChanged(object? sender, ItemChangeEventArgs e)
    {
        ...
        else if (e.ChangeType == ContentChangeType.CollectionRemove)
        {
            ...
            if (CollectionItemIdHelper.HasCollectionItemIds(collection))
            {
                overrideValue = node.BaseNode is not null && !UpdatingPropertyFromBase ? OverrideType.New : OverrideType.Base;
        ...
        if (objectNode is not null)
        {
            ...
            if ((assetNode.Descriptor is CollectionDescriptor || assetNode.Descriptor is DictionaryDescriptor) && CollectionItemIdHelper.HasCollectionItemIds(objectNode.Retrieve()))
            {
                ...
                foreach (var index in objectNode.Indices)
                {
                    ...
                    if (objectNode.IsItemOverridden(index) || (nodesToReset is not null && nodesToReset.TryGetValue(objectNode, out NodeIndex indexToReset) && indexToReset != index))
                        continue;
    ...
    private bool ShouldReconcileMember(AssetMemberNode memberNode, GraphNodePath currentPath, bool reconcileObjectReference, Dictionary<IGraphNode, NodeIndex>? nodesToReset)
    {
    ...
    private bool ShouldReconcileItem(IAssetObjectNode node, NodeIndex localIndex, NodeIndex baseIndex, bool reconcileObjectReference, Dictionary<IGraphNode, NodeIndex> nodesToReset)
    {
