...
namespace Stride.AssetCompiler
{
    public class PackageBuilder
    {
        ...
        private BuildResultCode BuildMaster()
        {
            ...
            CompilerCrashCapture crashCapture = null;
            try
            {
                ...
                // Capture crashes (unexpected exceptions escaping a command) to the on-disk store, unless disabled.
            ...
            finally
            {
                ...
                // After the build, do something with whatever crashed: pop the reporter (attended), send
                // headlessly (CI), or just leave the files on disk (unattended / save).
                HandleCapturedCrashes(crashCapture);
            }
        }
        ...
        private void HandleCapturedCrashes(CompilerCrashCapture crashCapture)
        {
            ...
            try
            {
                ...
                if (crashRun != null && !crashRun.IsEmpty)
                {
        ...
        // The reporter exe: an explicit override wins (dev/testing, odd deploy layouts), else the sibling exe.
        private static string ResolveCrashReporter()
        {
