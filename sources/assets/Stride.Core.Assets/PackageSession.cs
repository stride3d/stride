...
public abstract class PackageContainer
{
    ...
    public void Save(ILogger log, PackageSaveParameters? saveParameters = null)
    {
        ...
        saveParameters ??= PackageSaveParameters.Default();
        ...
        try
        {
            ...
            if (Package.IsDirty)
            {
                ...
                lock (filesToDelete)
                {
                    filesToDeleteLocal = [.. filesToDelete];
                    filesToDelete.Clear();
                }
                ...
                try
                {
                    ...
                    if (Package.PreviousPackagePath is not null && Package.PreviousPackagePath != Package.FullPath)
                    {
...
{
    protected SolutionProject(Package package) : base(package)
    {
    ...
    private void DirectDependencies_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        ...
        if (e.NewItems?.Count > 0)
        {
            foreach (DependencyRange dependency in e.NewItems)
            {
                ...
                {
                    case DependencyType.Package:
                        msbuildProject.AddItem("PackageReference", dependency.Name, [new KeyValuePair<string, string>("Version", dependency.VersionRange.ToString())])
...
public sealed partial class PackageSession : IDisposable, IAssetFinder
{
    ...
    /// </summary>
    public static readonly Version DefaultVisualStudioVersion = new("16.0.0.0");

    internal static readonly string SolutionHeader =
        """
        Microsoft Visual Studio Solution File, Format Version 12.00
        # Visual Studio 16
        VisualStudioVersion = {0}
        MinimumVisualStudioVersion = {0}
        """.ToFormat(DefaultVisualStudioVersion);

    private readonly Dictionary<Package, List<PendingPackageUpgrade>> pendingPackageUpgradesPerPackage = [];
    private readonly ConstraintProvider constraintProvider = new();
    private readonly PackageCollection packages;
    private readonly Dictionary<Package, Package> packagesCopy;
    private readonly object dependenciesLock = new();
    ...
    public AssetDependencyManager DependencyManager
    {
        get
        {
            lock (dependenciesLock)
            {
                return dependencies ??= new AssetDependencyManager(this);
            }
        }
    }
    ...
    public PackageContainer AddExistingProject(UFile projectPath, ILogger logger, PackageLoadParameters? loadParametersArg = null)
    {
        ...
        try
        {
            ...
            LoadMissingAssets(logger, [package], loadParameters);
    ...
    public AssetItem? FindAssetFromProxyObject(object? proxyObject)
    {
        ...
        return reference is not null ? (FindAsset(reference.Id) ?? FindAsset(reference.Url)) : null;
    }
    ...
    private PackageContainer LoadProject(ILogger log, string filePath, PackageLoadParameters? loadParameters = null)
    {
        ...
        if (vsPackage is not null && PackageSessionHelper.IsPackage(vsPackage, out var packagePathRelative))
        {
        ...
        if (package.Meta.Version is not null)
        {
    ...
    public static void Load(string filePath, PackageSessionResult sessionResult, PackageLoadParameters? loadParameters = null)
    {
        ...
        try
        {
            ...
            if (string.Equals(Path.GetExtension(filePath), ".sln", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                if (versionHeader is not null && Version.TryParse(versionHeader.Value, out var version))
                ...
                || Path.GetExtension(filePath).Equals(Package.PackageFileExtension, StringComparison.InvariantCultureIgnoreCase))
            {
    ...
    public void Save(ILogger log, PackageSaveParameters? saveParameters = null)
    {
        ...
        try
        {
            ...
            //    if (newPackage is not null && package.PackagePath is not null && newPackage.PackagePath != package.PackagePath)
            //    {
            ...
            foreach (var (assetPath, assetItem) in assetsOrPackagesToRemove)
            {
                try
                {
                    ...
                    if (projectFullPath is not null && assetItem.Asset is IProjectAsset projectAsset)
                    {
    ...
    private Dictionary<UFile, AssetItem> BuildAssetsOrPackagesToRemove()
    {
        ...
        var assetsOrPackagesToRemove = new Dictionary<UFile, AssetItem>();
        foreach (var assetIt in previousAssets)
        {
            var asset = assetIt.Value;

            if (!newAssets.TryGetValue(assetIt.Key, out var newAsset) || newAsset.Location != asset.Location)
            {
    ...
    private void ProjectsCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        switch (e.Action)
        {
            case NotifyCollectionChangedAction.Add:
                {
                    if (e.NewItems?[0] is PackageContainer project)
                        RegisterProject(project);
                }
                break;
            case NotifyCollectionChangedAction.Remove:
                {
                    if (e.OldItems?[0] is PackageContainer project)
    ...
    private bool TryLoadAssemblies(PackageSession session, ILogger log, Package package, PackageLoadParameters loadParameters, [MaybeNullWhen(false)] out AssetLoadingInfo info)
    {
        ...
        try
        {
            ...
            newLoadParameters.ExtraCompileProperties = [];
            if (loadParameters.ExtraCompileProperties is not null)
            {
    ...
    private static PackageUpgrader? CheckPackageUpgrade(ILogger log, Package dependentPackage, PackageDependency dependency, Package dependencyPackage)
    {
        ...
        if (dependencyPackage.Meta.Version > dependentPackagePreviousMinimumVersion)
        {
            ...
            if (packageUpgrader is not null)
            {
