...
public class AssetDependenciesCompiler
{
    ...
    private void Prepare(AssetCompilerResult finalResult, AssetCompilerContext context, AssetItem assetItem, Type compilationContext, HashSet<BuildAssetNode> visitedItems, Dictionary<AssetId, BuildStep> compiledItems, BuildStep? parentBuildStep = null, 
        BuildDependencyType dependencyType = BuildDependencyType.Runtime)
    {
        ...
        if (visitedItems.Add(assetNode))
        {
            ...
            if ((dependencyType & ~BuildDependencyType.CompileAsset) != 0 && assetBuildSteps is null)
            {
            ...
            if (assetBuildSteps is null)
                return;
        }

        // Link the created build steps to their parent step.
        if (parentBuildStep is not null && assetBuildSteps is not null && (dependencyType & BuildDependencyType.CompileContent) == BuildDependencyType.CompileContent) //only if content is required Content.Load
