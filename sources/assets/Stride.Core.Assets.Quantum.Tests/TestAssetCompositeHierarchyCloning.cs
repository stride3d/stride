...
public class TestAssetCompositeHierarchyCloning
{
    ...
    public void TestSimpleCloneSubHierarchy()
    {
        ...
        var clone = AssetCompositeHierarchyPropertyGraph<Types.MyPartDesign, Types.MyPart>.CloneSubHierarchies(graph.Container.NodeContainer, graph.Asset, originalRoot.Part.Id.Yield(), SubHierarchyCloneFlags.None, out var remapping);
    ...
    {
        var graph = AssetHierarchyHelper.BuildAssetAndGraph(2, 2, 2, x => x.Parts[GuidGenerator.Get(5)].Part.MyReferences = [x.Parts[GuidGenerator.Get(2)].Part]);
        Debug.Write(AssetHierarchyHelper.PrintHierarchy(graph.Asset));
        var originalRoot = graph.Asset.Hierarchy.Parts[graph.Asset.Hierarchy.RootParts[1].Id];
        var clone = AssetCompositeHierarchyPropertyGraph<Types.MyPartDesign, Types.MyPart>.CloneSubHierarchies(graph.Container.NodeContainer, graph.Asset, originalRoot.Part.Id.Yield(), SubHierarchyCloneFlags.None, out var remapping);
        var cloneRoot = clone.Parts[clone.RootParts.Single().Id];
    ...
    public void TestCloneSubHierarchyWithCleanExternalReferences()
    {
        ...
        Debug.Write(AssetHierarchyHelper.PrintHierarchy(graph.Asset));
        var originalRoot = graph.Asset.Hierarchy.Parts[graph.Asset.Hierarchy.RootParts[1].Id];
        var clone = AssetCompositeHierarchyPropertyGraph<Types.MyPartDesign, Types.MyPart>.CloneSubHierarchies(graph.Container.NodeContainer, graph.Asset, originalRoot.Part.Id.Yield(), SubHierarchyCloneFlags.CleanExternalReferences, out var remapping);
        var cloneRoot = clone.Parts[clone.RootParts.Single().Id];
        Assert.Empty(remapping);
        Assert.Equal(3, clone.Parts.Count);
        Assert.Single(clone.RootParts);
    ...
    public void TestCloneSubHierarchyWithGenerateNewIdsForIdentifiableObjects()
    {
        ...
        Debug.Write(AssetHierarchyHelper.PrintHierarchy(graph.Asset));
        var originalRoot = graph.Asset.Hierarchy.Parts[graph.Asset.Hierarchy.RootParts[1].Id];
        var clone = AssetCompositeHierarchyPropertyGraph<Types.MyPartDesign, Types.MyPart>.CloneSubHierarchies(graph.Container.NodeContainer, graph.Asset, originalRoot.Part.Id.Yield(), SubHierarchyCloneFlags.GenerateNewIdsForIdentifiableObjects, out var remapping);
        var cloneRoot = clone.Parts[clone.RootParts.Single().Id];
    ...
    public void TestCloneSubHierarchyInstanceWithoutRemoveOverrides()
    {
        ...
        var clone = AssetCompositeHierarchyPropertyGraph<Types.MyPartDesign, Types.MyPart>.CloneSubHierarchies(container, derivedAsset.Asset, originalRoot.Id.Yield(), SubHierarchyCloneFlags.None, out var remapping);
    ...
    public void TestCloneSubHierarchyInstanceWithRemoveOverrides()
    {
        ...
        Debug.Write(AssetHierarchyHelper.PrintHierarchy(derivedAsset.Asset));
        var originalRoot = derivedAsset.Asset.Hierarchy.RootParts.Single();
        var clone = AssetCompositeHierarchyPropertyGraph<Types.MyPartDesign, Types.MyPart>.CloneSubHierarchies(container, derivedAsset.Asset, originalRoot.Id.Yield(), SubHierarchyCloneFlags.RemoveOverrides, out var remapping);
        var cloneAsset = AssetHierarchyHelper.BuildAssetContainer(0, 0, 0, baseAsset.Container);
        var cloneRoot = clone.Parts[clone.RootParts.Single().Id];
        cloneAsset.Graph.AddPartToAsset(clone.Parts, cloneRoot, null, 0);
        cloneAsset.Graph.RefreshBase();
