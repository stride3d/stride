...
#if XENKO_GRAPHICS_API_DIRECT3D11 || XENKO_GRAPHICS_API_OPENGL
using System;
                ...
                for (int index = 0; index < bindingOperations.Length; index++, bindingOperation = Interop.IncrementPinned(bindingOperation))
                {
                    ...
                    switch (bindingOperation.Class)
                    {
                        ...
                        case EffectParameterClass.UnorderedAccessView:
                            {
                                commandList.SetUnorderedAccessView(bindingOperation.Stage, bindingOperation.SlotStart, (GraphicsResource)value.Value, value.UAVInitialOffset);
                                break;
                            }
                        default:
