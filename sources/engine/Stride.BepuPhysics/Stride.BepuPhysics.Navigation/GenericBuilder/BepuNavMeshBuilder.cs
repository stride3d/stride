...
public class BepuNavMeshBuilder
{
    ...
    /// <returns></returns>
    internal static DtNavMesh CreateNavMesh(RcNavMeshBuildSettings navSettings, AsyncMeshInput input, int threads, CancellationToken cancelToken)
    {
    ...
    /// <param name="cancelToken"></param>
    /// <returns></returns>
    internal static List<RcBuilderResult> CreateBuildResults(RcNavMeshBuildSettings navSettings, AsyncMeshInput input, int threads, CancellationToken cancelToken)
    {
        var verts = new List<VertexPosition3>();
        var indices = new List<int>();
        for (int collidableI = 0, shapeI = 0; collidableI < input.Matrices.Count; collidableI++)
        ...
        return NavMeshBuilder.GetBuildResults(navSettings, geom, threads, cancelToken);
    }
}
