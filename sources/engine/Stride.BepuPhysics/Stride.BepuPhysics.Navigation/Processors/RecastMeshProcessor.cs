...
public class RecastMeshProcessor : EntityProcessor<BepuNavigationBoundingBoxComponent>
{
    ...
    {
        var shapes = containerData.GetShapeData();
        ...
        for (int i = 0; i < shapes.Count; i++)
        {
            ...
            {
                shapes[i].Points[ii] = Vector3.Transform(shapes[i].Points[ii], containerData.Entity.Transform.WorldMatrix).XYZ();
            }
            AppendArrays(shapes[i].Points.ToArray(), shapes[i].Indices.ToArray());
        }
    }
