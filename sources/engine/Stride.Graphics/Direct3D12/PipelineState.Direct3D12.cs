    ...
    public unsafe partial class PipelineState
    {
        ...
        internal PipelineState(GraphicsDevice graphicsDevice, PipelineStateDescription pipelineStateDescription) : base(graphicsDevice)
        {
            ...
            else // Graphics Pipeline State
            {
                ...
                {
                    InputLayout = PrepareInputLayout(pipelineStateDescription.InputElements, effectReflection.InputAttributes),
                    PRootSignature = rootSignature,
                                              ...
                                              ref int descriptorSamplerOffset)
            {
                ...
                {
                    if (binding.KeyInfo.Key != layoutBuilderEntry.Key)
                        continue;
                    ...
                    if (isSampler)
                    {
                        if (layoutBuilderEntry.ImmutableSampler is not null)
                        {
                            StaticSamplerDesc samplerDesc = new()
                            {
                                ...
                                // Note: Until we process reflection on the spirv_to_dxil bytecode, we don't know which stage use what so make it visible to all stage for now
                                ShaderVisibility = ShaderVisibility.All, //GetShaderVisibilityForStage(binding.Stage),
                                         ...
                                         ref int layoutBindCount)
            {
                foreach ((ShaderStage stage, List<DescriptorRange> descriptorRanges) in descriptionRangesToPrepare)
                {
                    ...
                    {
                        // Note: Until we process reflection on the spirv_to_dxil bytecode, we don't know which stage use what so make it visible to all stage for now
                        ShaderVisibility = ShaderVisibility.All, //GetShaderVisibilityForStage(stage),
            ...
            InputLayoutDesc PrepareInputLayout(InputElementDescription[] inputElements, List<ShaderInputAttributeDescription> inputAttributes)
            {
                ...
                var pTexCoordSemantic = SilkMarshal.StringToPtr("TEXCOORD");

                var dstInputElementsCount = 0;
                for (int i = 0; i < inputElements.Length; ++i)
                {
                    ref var srcInputElement = ref pipelineStateDescription.InputElements[i];

                    tempMemoryAllocations.Add(pTexCoordSemantic);

                    // Find matching element in input attributes
                    // Note: in our shader generated by spirv_to_dxil, semantic info is gone
                    // and locations will be renamed TEXCOORDX with X being the inputAttribute.Location
                    foreach (var inputAttribute in inputAttributes)
                    {
                        if (inputAttribute.SemanticName == srcInputElement.SemanticName && inputAttribute.SemanticIndex == srcInputElement.SemanticIndex)
                        {
                            dstInputElements[dstInputElementsCount++] = new InputElementDesc
                            {
                                Format = (Format)srcInputElement.Format,
                                AlignedByteOffset = (uint)srcInputElement.AlignedByteOffset,
                                SemanticName = (byte*)pTexCoordSemantic,
                                SemanticIndex = (uint)inputAttribute.Location,
                ...
                {
                    NumElements = (uint)dstInputElementsCount,
