...
namespace Stride.Assets.Presentation.Templates
{
    public class ColliderShapeStaticMeshFactoryTemplateGenerator : AssetFactoryTemplateGenerator
    {
        ...
        protected override async Task<bool> PrepareAssetCreation(AssetTemplateGeneratorParameters parameters)
        {
            ...
            var modelViewModel = await BrowseForAsset(parameters.Package, new[] { typeof(IModelAsset) }, new UFile(parameters.Name).GetFullDirectory(), "Select a model to use to generate the static mesh - _cancel to leave the model empty_");
