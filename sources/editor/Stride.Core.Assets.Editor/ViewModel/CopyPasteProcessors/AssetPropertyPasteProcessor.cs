...
namespace Stride.Core.Assets.Editor.ViewModel.CopyPasteProcessors
{
    ...
    public class AssetPropertyPasteProcessor : PasteProcessorBase
    {
        ...
        private void Paste([NotNull] IPasteItem pasteResultItem, IGraphNode targetNode, NodeIndex index, bool replace)
        {
            ...
            else if (targetNode.Descriptor.Category == DescriptorCategory.List)
            {
                ...
                if (replace)
                {
                    ...
                    if (index.IsEmpty)
                    {
                        ...
                        var count = targetListDescriptor.GetListCount(targetNode.Retrieve());
                ...
                else
                {
                    ...
                    {
                        // Add the new items
                        var i = targetListDescriptor.GetListCount(targetNode.Retrieve());
                        foreach (var item in EnumerateItems(copiedData, copiedDataDescriptor))
                        {
                            var itemIndex = new NodeIndex(i);
                            if (CanInsertItem(collectionNode, itemIndex, item))
            ...
            else if (targetNode.Descriptor.Category == DescriptorCategory.Set)
            {
                ...
                if (replace)
                {
                    ...
                    {
                        // First clear the collection
                        var count = targetSetDescriptor.GetSetCount(targetNode.Retrieve());
                        for (var j = count - 1; j >= 0; j--)
                        {
                            var itemIndex = new NodeIndex(j);
                            if (CanRemoveItem(collectionNode, itemIndex))
                ...
                else
                {
                    ...
                    {
                        // Add the new items
                        var i = targetSetDescriptor.GetSetCount(targetNode.Retrieve());
                        foreach (var item in EnumerateItems(copiedData, copiedDataDescriptor))
                        {
                            var itemIndex = new NodeIndex(i);
                            if (CanInsertItem(collectionNode, itemIndex, item))
        ...
        {
            var count = listDescriptor.GetListCount(list);
        ...
        private static bool TryConvertListData([NotNull] object sourceList, [NotNull] ListDescriptor listDescriptor, out object convertedList)
        {
            try
            {
                ...
                return listDescriptor.GetListCount(convertedList) > 0;
            }
            catch (InvalidCastException) { }
            catch (InvalidOperationException) { }
            catch (FormatException) { }
        ...
        private static bool TryConvertSetData([NotNull] object sourceSet, [NotNull] SetDescriptor setDescriptor, out object convertedSet)
        {
            try
            {
                ...
                return setDescriptor.GetSetCount(convertedSet) > 0;
            }
            catch (InvalidCastException) { }
            catch (InvalidOperationException) { }
            catch (FormatException) { }
