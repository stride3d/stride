    ...
    public abstract class EditorViewModel : ViewModelBase
    {
        ...
        public async Task<bool?> OpenSession(UFile filePath)
        {
            ...
            if (filePath != null && !File.Exists(filePath))
            {
                ...
                await ServiceProvider.Get<IDialogService2>().MessageBox(string.Format(Tr._p("Message", @"The file '{0}' does not exist."), filePath), MessageBoxButton.OK, MessageBoxImage.Information);
                return false;
            }
        ...
        public async Task OpenFile(string filePath, bool tryEdit)
        {
            try
            {
                ...
                {
                    await ServiceProvider.Get<IDialogService2>().MessageBox(Tr._p("Message", "You need to save the file before you can open it."), MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
