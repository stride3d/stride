...
public class AssetPropertyPasteProcessor : PasteProcessorBase
{
    ...
    public override bool ProcessDeserializedData(AssetPropertyGraphContainer graphContainer, object targetRootObject, Type targetMemberType, ref object data, bool isRootDataObjectReference, AssetId? sourceId, YamlAssetMetadata<OverrideType> overrides, YamlAssetPath basePath)
    {
        ...
        try
        {
            ...
            data = AssetCloner.Clone<object>(data, AssetClonerFlags.GenerateNewIdsForIdentifiableObjects, externalReferences, out var idRemapping);
        }
    ...
    {
        propertyContainer.TryGetValue(IsReplaceKey, out var replace);
    ...
    private void Paste(IPasteItem pasteResultItem, IGraphNode targetNode, NodeIndex index, bool replace)
    {
    ...
    private static bool ConvertForCollection(CollectionDescriptor collectionDescriptor, ref object data)
    {
        ...
        {
            if (!TypeConverterHelper.TryConvert(data, collectionDescriptor.ElementType, out var convertedData))
                return false;
    ...
    private static bool ConvertForList(ListDescriptor listDescriptor, ref object data)
    {
        ...
        {
            if (!TypeConverterHelper.TryConvert(data, listDescriptor.ElementType, out var convertedData))
                return false;
    ...
    private static bool ConvertForSet(SetDescriptor setDescriptor, ref object data)
    {
        ...
        {
            if (!TypeConverterHelper.TryConvert(data, setDescriptor.ElementType, out var convertedData))
                return false;
    ...
    private static IEnumerable EnumerateList(object list, ListDescriptor listDescriptor)
    {
    ...
    private static IEnumerable EnumerateCollection(object collection, CollectionDescriptor collectionDescriptor)
    {
    ...
    private static IEnumerable EnumerateItems(object collectionOrSingleItem, ITypeDescriptor typeDescriptor)
    {
    ...
    private static bool TryConvertCollectionData(object sourceCollection, CollectionDescriptor collectionDescriptor, out object convertedCollection)
    {
        try
        {
            ...
            {
                if (!TypeConverterHelper.TryConvert(item, collectionDescriptor.ElementType, out var obj))
                {
    ...
    private static bool TryConvertListData(object sourceList, ListDescriptor listDescriptor, out object convertedList)
    {
        try
        {
            ...
            {
                if (!TypeConverterHelper.TryConvert(item, listDescriptor.ElementType, out var obj))
                {
                    // (optimistic) try to convert the remaining items
                    continue;
                }
    ...
    private static bool TryConvertSetData(object sourceSet, SetDescriptor setDescriptor, out object convertedSet)
    {
        try
        {
            ...
            {
                if (!TypeConverterHelper.TryConvert(item, setDescriptor.ElementType, out var obj))
                {
                    // (optimistic) try to convert the remaining items
                    continue;
                }
    ...
    private static bool TryConvertDictionaryData(object sourceDictionary, DictionaryDescriptor dictionaryDescriptor, out object convertedDictionary)
    {
        try
        {
