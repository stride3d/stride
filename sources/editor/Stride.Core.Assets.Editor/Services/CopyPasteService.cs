...
internal sealed class CopyPasteService : ICopyPasteService
{
    ...
    private static Type GetDataType(string text)
    {
        try
        {
    ...
    private IEnumerable<ICopyProcessor> GetValidCopyProcessors(Type dataType)
    {
    ...
    private IEnumerable<IPasteProcessor> GetValidPasteProcessors(Type targetRootType, Type targetMemberType, Type pastedDataType)
    {
    ...
    private static string SerializeData(CopyPasteData data, AttachedYamlAssetMetadata metadata)
    {
        var stream = new MemoryStream();
