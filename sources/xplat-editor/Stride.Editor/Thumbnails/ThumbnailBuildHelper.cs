    ...
    public static void ApplyThumbnailStatus(Image thumbnailImage, LogMessageType dependencyBuildStatus)
    {
        ...
        using (var thumbnailBuilderHelper = new ThumbnailBuildHelper())
        {
            if (errorTexture == null)
            {
                ...
                errorTexture = Texture.New(thumbnailBuilderHelper.GraphicsDevice, DefaultThumbnails.DependencyError);
                warningTexture = Texture.New(thumbnailBuilderHelper.GraphicsDevice, DefaultThumbnails.DependencyWarning);
            }
