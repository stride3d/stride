...
public sealed class MatrixEditor : VectorEditorBase<Matrix?>
{
    ...
    protected override Matrix? UpdateValueFromComponent(AvaloniaProperty property)
    {
        ...
        foreach (var (dependencyProperty, index) in PropertyToIndex)
        {
            array[index] = property == dependencyProperty ? ((float?)GetValue(dependencyProperty)).Value : Value.Value[index];
        }
