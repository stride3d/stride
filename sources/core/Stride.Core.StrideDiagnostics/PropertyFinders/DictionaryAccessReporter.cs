...
public class DictionaryAccessReporter : IViolationReporter
{
    ...
    {
        if (classMember is IPropertySymbol property && PropertyHelper.IsDictionary(property, ClassInfo) && !PropertyHelper.IsArray(property) && !this.ShouldBeIgnored(property))
        {
            return true;
        }
        return false;
