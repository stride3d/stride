...
namespace CSharpBeginner.Code
{
    ...
    {
        private Model _loadedMannequinModel = null;
        private Stack<Entity> _spawnedEntities = new Stack<Entity>();
        ...
        public override void Update()
        {
            ...
            {
                _loadedMannequinModel = Content.Load<Model>("Models/mannequinModel");
            }
            ...
            {
                Content.Unload(_loadedMannequinModel);
                _loadedMannequinModel = null;
            }
            ...
            if (Input.IsKeyPressed(Keys.C))
            {
                ...
                {
                    Entity.Scene.Entities.Remove(_spawnedEntities.Pop());
                }
            }

            DebugText.Print("Model is " + (_loadedMannequinModel == null ? "not loaded": "loaded"), new Int2(340, 580));
            ...
            DebugText.Print("Spawned entities count: " + _spawnedEntities.Count, new Int2(340, 700));
        }

        private void CreateEntityWithModelAndRandomlyPositionInScene()
        {
            if (_loadedMannequinModel != null)
            {
                // Create a new model component that references the loaded mannequin model
                var modelComponent = new ModelComponent(_loadedMannequinModel);

                // Get a random position near the center of the scene
                var randomPosition = new Vector3(_random.Next(-2, 4), 0, _random.Next(-2, 2));
