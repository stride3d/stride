...
- **Cite every source you fold in.** When one bullet groups work from more than one PR or commit,
  ...
  `### 🔧 Under the hood`: collapse freely into a few bullets, but each bullet still lists every
  PR it summarizes, so no contributor loses credit. Every PR in the authoritative change list must
  be referenced somewhere in the output — as its own bullet or inside a grouped bullet's list.
