...
| `Stride.CrashReporter` | Out-of-process Avalonia reporter exe. On a native crash the GUI hosts' native trigger spawns it in `--capture` mode: it writes the dump from *outside* the dying process, records the crash, and — when attended — shows the report. Also shows crashes routed to it by a host, and GameStudio's own **managed** crashes: GameStudio saves the report and a triage dump, spawns the reporter with its pid (`--host-pid`) and stays alive, blocked, until it closes, so the reporter can write a full memory dump of the live GameStudio on demand. Shipped as a NuGet store package the hosts resolve. |
