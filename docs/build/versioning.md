...
A samples release, start to finish (the engine release never touches `SamplesVersion`; the number moves only here):

1. **Pick the next content number**: the committed `SamplesVersion` plus one on the patch part (`4.4.0` → `4.4.1`), or the first stable number of the line (`4.4.0-beta7` → `4.4.0`). The major.minor is the engine line's; the patch part is a counter with no relation to the engine's patch (engine `4.4.0-beta1` can name samples `4.4.0`, and engine `4.4.0` samples `4.4.1`). Stable unless the next engine release is a prerelease.
2. **Run `PrepareSamplesForRelease`** with that number (below). It migrates the samples, writes the number into `SharedAssemblyInfo.cs` and packs the content as this checkout's `-devN`.
3. **Open the samples in GameStudio** and fix what the upgraders left (they reference this checkout's dev engine at this point). Any real change goes through the normal PR review with the rest.
4. **Run `SamplesToReleaseEngine`** and commit the samples together with the `SamplesVersion` bump, one commit. Open the PR; its CI packs the content and runs the template smoke tests and the sample screenshot tests on it.
5. **After the merge, dispatch [`release-samples.yml`](../../.github/workflows/release-samples.yml)** on the branch with `sign` and `deploy`, and the `version-suffix` of the engine release the content precedes (empty for a stable engine). It publishes the three packages and tags `samples/<version>`.
6. **Then release the engine.** Its deploy checks that the content it names is on the feed.

Bumping `SamplesVersion` means "a new samples release", and it is committed together with the migrated samples. `PrepareSamplesForRelease` does the mechanical part: guards on the number, `UpgradeSamples` (every sample through the **real package upgraders**: migrate assets to the current format, rewrite `Stride.*` to this checkout's dev build, restore from the local dev feed), `SamplesToReleaseEngine`, the new number written into `SharedAssemblyInfo.cs`, and a `-devN` pack of the content. It then prints the part that stays manual on purpose: open the samples in GameStudio, fix what the upgraders left, commit.

```bash
dotnet msbuild build/Stride.Samples.build -t:PrepareSamplesForRelease -p:StrideSamplesVersion=4.4.1   # -p:SamplesEngineVersion= for the engine version the samples reference
```

Publishing is [`release-samples.yml`](../../.github/workflows/release-samples.yml) (manual dispatch, `stride-release-managers` only): `PackageSamples` in `build/Stride.build` packs the three content packages release-shaped (fully preprocessed) and stamped for this branch's engine version plus the `version-suffix` input, the template smoke tests run against the repo build, the packs are signed and, when `deploy`, pushed, tagged `samples/<version>` and given a GitHub Release. It refuses a version that is already on the feed (samples changed without a new content version).

**Order**: merge the prepared samples release → publish the content → release the engine that names it. Content on the feed that no engine names yet is harmless; an engine naming content that is not on the feed breaks every New Project, so `release-deploy.yml` refuses to deploy an engine whose `SamplesVersion` is not published, or that is stable while naming prerelease content. Samples changed since the number was set only produce a warning, with the file list in the run summary: the engine ships with the published content, which is fine for work not released yet, and the release manager decides whether the new engine needs a samples release first. A PR that changes the samples without a new `SamplesVersion` gets a warning from the change-detection job ([`changes.yml`](../../.github/workflows/changes.yml)), so the needed samples release is visible long before the engine release; local builds never check this (local content packs are always `-devN`).

The test projects that instantiate templates (`Stride.Templates.Tests`, `tests/editor`, `tests/enduser/Stride.Samples.Tests` and `Stride.Packaging.Tests`) opt the content packs in on their template references: they test this checkout's samples on this checkout's engine, and they don't depend on the content being published.
